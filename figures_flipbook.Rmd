...
cowplot::plot_grid(plot.mass.re, plot.gs.re, align = "h")
...
lab <- dat3_nonas %>%
  ...
  mutate(meanspeed = mean(sline_speed_mm_s),
         sd = sd(sline_speed_mm_s)) %>%
...
lab %>%
  ggplot(aes(x = jitter(avg_temp, amount = 0.2), y = meanspeed))+
  geom_point(aes(col = treatment))+
  geom_smooth(method = "lm", se = F, aes(group = pond),
              col = "black", lwd = 0.3, lty = "dashed")+
  geom_point(data = wild, aes(x = avg_pond_temp, y = meanspeed))+
  geom_smooth(data = wild, aes(x = avg_pond_temp, y = meanspeed),
              method = "lm", se = F, col = "black", lwd = 0.3)+
  geom_errorbar
  theme_minimal()+
