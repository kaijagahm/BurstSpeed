...
loggers_cropped %>% filter(pond == "CPS")
...
dat3_nonas_std <- dat3_nonas_std %>% mutate(
  fit.re = predict(consensus_mod_std, newdata = new.lab, re.form = NULL),
  resid = summary(consensus_mod_std)$resid,
  fit = fit.re + resid)

# Explore reaction norms by clutch and pond
dat3_nonas_std %>% group_by(treatment, pond, clutch, tadpole) %>% 
  summarise(fit = mean(fit)) %>%
  group_by(treatment, pond, clutch) %>% 
  summarise(fit = mean(fit)) %>%
  ggplot(aes(x = treatment, y = fit)) +
  geom_point() +
  geom_line(aes(group = interaction(pond, clutch)), size = 1) +
  facet_wrap(~pond)
...
# Fit the predicted speed for each individual, since we held the fixed effects constant, these are essentially just the conditional modes of the random effects. We also want to maintain our individual level variation, so we include the individual residuals from the model.
wild_nonas_std <- wild_nonas_std %>% mutate(
...
wild_nonas_std %>% group_by(pond, tadpole, avg_pond_temp) %>% 
  summarise(fit = mean(fit)) %>%
...
```{r}
lab <- dat3_nonas_std %>%
  group_by(treatment, pond, clutch, tadpole, avg_temp) %>% summarize(fit = mean(fit)) %>%
  group_by(treatment, pond, clutch, avg_temp) %>% summarize(fit = mean(fit)) %>%
  group_by(treatment, pond, avg_temp) %>% summarize(fit = mean(fit))

wild <- wild_nonas_std %>%
  group_by(pond, tadpole, avg_pond_temp) %>% summarize(fit = mean(fit)) %>%
  group_by(pond, avg_pond_temp) %>% summarize(fit = mean(fit))
...
lab %>%
  ...
  geom_point(data = wild, aes(x = avg_pond_temp, y = fit), color = "black", size = 1.5)+
  geom_smooth(data = wild, aes(x = avg_pond_temp, y = fit), method = "lm", se = T, color = "black")+
  theme_minimal()+
  theme(legend.position = "bottom")+
