...
# Model predictions (gs): lab-reared
```{r}
# Residual of mass on gosner stage
...
```{r}
load("data/outputs/newlab.mass.Rda")
load("data/outputs/newwld.mass.Rda")
load("data/outputs/newlab.gs.Rda")
load("data/outputs/newwld.gs.Rda")

new.mass <- bind_rows(newlab.mass, newwld.mass) %>%
  mutate(treatment = case_when(is.na(treatment) ~ "Wild"))
new.gs <- bind_rows(newlab.gs, newwld.gs) %>%
  mutate(treatment = case_when(is.na(treatment) ~ "Wild"))

# TRY TO RECREATE THIS FIGURE WITH THE DATASET BOUND TOGETHER, TO FACILITATE MAKING THE LEGEND
