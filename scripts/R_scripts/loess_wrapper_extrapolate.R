...
#----------------------------------------------------------------------#
# MAKE SURE TO OMIT NA'S IN YOUR DATA BEFORE USING, OTHERWISE THIS WILL THROW MYSTERIOUS ERRORS!
library(bootstrap)
...
library(bootstrap)
loess_bootstrap_partial <- function (x, y, span.vals = seq(0.25, 1, by = 0.05), folds = 5){
  # Do model selection using mean absolute error, which is more robust than squared error.
  mean.abs.error <- numeric(length(span.vals))
