...
scores <- scores %>% mutate(size = sizes)

# Size vs. PC1
scores %>% filter(size > 3.5) %>% ggplot(aes(x = size, y = PC1, col = treatment))+
  geom_point() +
...
# Size vs. PC2
scores %>% filter(size > 3.5) %>% ggplot(aes(x = size, y = PC2, col = treatment))+
  geom_point() +
  geom_smooth(method = "lm") +
  ggtitle("Size vs. PC2")+
