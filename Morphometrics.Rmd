...
allmorpho %>% filter(size > 3.5, !is.na(gs)) %>% ggplot(aes(x = size, y = PC1, col = gs))+
  ...
  ylab("PC2 (~ head length:tail length)")+
  xlab("Centroid Size") +
  labs(col = "Gosner Stage")

# Size vs. PC2, with GS
allmorpho %>% filter(size > 3.5, !is.na(gs)) %>% ggplot(aes(x = size, y = PC2, col = gs))+
  geom_smooth(size = 0.5, colour = "black", method = "lm")+
  geom_point()+
  scale_color_viridis()+
  facet_wrap(~treatment)+
  ggtitle("Size vs. PC2, by treatment and GS")+
  ylab("PC2 (~ depth:length)")+
  xlab("Centroid Size") +
  labs(col = "Gosner Stage")
```
