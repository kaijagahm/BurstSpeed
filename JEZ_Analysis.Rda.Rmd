...
visreg(performance_mod_wild_std, xvar = "pond")
...
### Model
```{r}
tradeoff_mod_wild <- lmer(fit ~ devrate + (1 | pond), data = wild_tradeoff)
summary(tradeoff_mod_wild)
visreg(tradeoff_mod_wild, by = "pond")
```
...
wild_tradeoff %>%
  ...
  geom_point(data = lab_tradeoff %>%
               ...
               group_by(treatment, pond) %>%
               summarize(devrate = mean(devrate), fit = mean(fit)),
             aes(x = devrate, y = fit, col = treatment),
             size = 3, alpha = 0.7)+
  geom_line(data = lab_tradeoff %>%
               group_by(treatment, pond, clutch, tadpole) %>%
               summarize(devrate = mean(devrate), fit = mean(fit)) %>%
               group_by(treatment, pond, clutch) %>%
               summarize(devrate = mean(devrate), fit = mean(fit)) %>%
               group_by(treatment, pond) %>%
               summarize(devrate = mean(devrate), fit = mean(fit)),
            aes(group = pond), lty = "dashed", lwd = 0.5, col = "black")+
  scale_color_manual(name = "Temperature treatment", values = c("orange", "dodgerblue3"))+
  theme_minimal()+
  theme(legend.position = "bottom")+
  ylab("Predicted burst speed (mm/s)")+
...
dev_temp_wild <- lmer(devrate ~ avg_pond_temp + (1 | pond), data = wild)
summary(dev_temp_wild)
visreg(dev_temp_wild)

# (Size at stage vs. pond temp?)
ssmodtemp <- lmer(mass_at_stage ~ avg_pond_temp + (1 | pond), data = wild)
summary(ssmodtemp)
visreg(ssmodtemp)
```