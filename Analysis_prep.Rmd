...
accels_allcrops %>% filter(tadpole == tadpoles[100], crop == "First half second") %>%
  ggplot(aes(x = frame_burst, y = accelma5, col = factor(trial)))+
  geom_line()+
  xlab("Frame")+
  ylab("Acceleration (5-frame moving average)")
```
...
```{r}
stats_fast3 %>% ggplot(aes(x = camera, y = sline_speed_m_s))+
  geom_violin()

stats_fast3 %>% ggplot(aes(x = T_initial, y = sline_speed_m_s, col = camera))+
  geom_point()+
  geom_smooth(method = "lm")
```
