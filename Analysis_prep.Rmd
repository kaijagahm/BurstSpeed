...
### Fastest 1 trial
```{r}
stats_fast1 <- stats %>% group_by(tadpole) %>% filter(sline_speed_m_s == max(sline_speed_m_s))
```

### Fastest 3 trials
```{r}
stats_fast3 <- stats %>% group_by(tadpole) %>% arrange(desc(sline_speed_m_s), .by_group = T) %>% slice(1:3)
