...
mass_gs_mod <- lm(mass ~ gs, data = dat3_nonas_std)
dat3_nonas_std <- dat3_nonas_std %>%
  mutate(mass_gs_resid = resid(mass_gs_mod))

resid <- lmer(sline_speed_mm_s ~ mass_gs_resid*treatment + gs*treatment + PC2 + T_initial + (1|pond/clutch/tadpole), data = dat3_nonas_std, REML = F)
summary(resid) 
