...
```{r}
dat3_nonas <- dat3_nonas %>% filter(!is.na(mass))
```
...
full <- lmer(sline_speed_mm_s ~ PC2*mass*gs*treatment + T_initial + (1|clutch/indiv),
             data = dat3_nonas_std, na.action = "na.fail", REML = F)
...
m1 <- lmer(sline_speed_mm_s ~ mass*treatment + gs + T_initial + 
             (1 | clutch/indiv), data = dat3_nonas_std, REML = F, 
    na.action = "na.fail")
...
m5 <- lmer(sline_speed_mm_s ~ mass*treatment + gs*mass + PC2 + T_initial + 
             (1 | clutch/indiv), data = dat3_nonas_std, REML = F, na.action = "na.fail")
...
```{r}
newdf <- dat3_nonas_std %>% select(treatment, clutch, indiv) # select grouping cols
newdf_means <- dat3_nonas_std %>% summarize(mass = mean(mass), # take mean of each
                                        gs = mean(gs),
