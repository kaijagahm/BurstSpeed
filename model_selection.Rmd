...
mass_gs_mod_std <- lm(mass ~ gs, data = dat3_nonas_std)
dat3_nonas_std <- dat3_nonas_std %>%
  mutate(size_at_stage = resid(mass_gs_mod_std))

resid <- lmer(sline_speed_mm_s ~ size_at_stage*treatment + gs*treatment + PC2 + T_initial + (1|pond/clutch/tadpole), data = dat3_nonas_std, REML = F)
summary(resid) 
...
# Predict mass from gosner stage (unstandardized)
mass_gs_mod <- lm(mass ~ gs, data = dat3_nonas)
dat3_nonas <- dat3_nonas %>%
  mutate(size_at_stage = resid(mass_gs_mod))

consensus_mod <- lmer(sline_speed_mm_s ~ size_at_stage*treatment + gs*treatment + PC2 + T_initial + (1|pond/clutch/tadpole), data = dat3_nonas)
...
```{r}
# Predict mass from gosner stage
mass_gs_mod_wild_std <- lm(mass ~ gs, data = wild_nonas_std)
wild_nonas_std <- wild_nonas_std %>%
  mutate(size_at_stage = resid(mass_gs_mod_wild_std))

consensus_mod_wild_std <- lmer(sline_speed_mm_s ~ size_at_stage*avg_pond_temp + gs*avg_pond_temp + PC2 + T_initial + (1|pond/tadpole), data = wild_nonas_std)
...
wild_nonas <- wild_nonas %>%
  mutate(size_at_stage = resid(mass_gs_mod_wild))

consensus_mod_wild <- lmer(sline_speed_mm_s ~ size_at_stage*avg_pond_temp + gs*avg_pond_temp + PC2 + T_initial + (1|pond/tadpole), data = wild_nonas)
