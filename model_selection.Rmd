...
```{r}
gs_mod <- mgcv::gam(gs ~ size + mass, data = dat3_nonas)
summary(gs_mod)

dat3_nonas %<>% mutate(gs_pred = predict(gs_mod, newdata = dat3_nonas))

dat3_nonas %>% ggplot(aes(x = gs_pred, y = mass))+
  geom_point()+
...
## Save model
```{r}
save(mod_gs, file = "data/outputs/mod_gs.Rda")
```
...
plot(mod_gs) # looks roughly normal, but just to make sure, we'll do individual plots
