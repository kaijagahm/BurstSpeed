...
## Fit full models
```{r}
fullmodel <- lmer(sline_speed_m_s ~ gs*PC1*PC2*size*mass*treatment + T_initial + (1|clutch/indiv), data = dat3_nonas, na.action = "na.fail", REML = FALSE)
summary(fullmodel) # we need to get rid of some of these
vif(fullmodel) # getting large

shape <- lmer(sline_speed_m_s ~ PC1*PC2*treatment + T_initial + (1|clutch/indiv), data = dat3_nonas, na.action = "na.fail")
summary(shape)

sizemass <- lmer(sline_speed_m_s ~ size*mass*treatment + T_initial + (1|clutch/indiv), data = dat3_nonas, na.action = "na.fail")
summary(sizemass)
...
dredge_full %>% ggplot(aes(x = 1:nrow(.), y = AICc))+
  geom_point()
