...
```{r}
dat3 <- stats_fast3 %>% select(tadpole, trial, indiv, treatment, sline_speed_m_s, PC1, PC2, pond, size, clutch, mass, gs, T_initial, camera) %>% ungroup() %>% mutate(treatment = factor(treatment),
                     pond = factor(pond),
                     clutch = factor(clutch)) %>%
  filter(treatment %in% c("High", "Low"), !is.na(PC1)) %>% droplevels()

# wild tadpoles are separate
wild <- stats_fast3 %>% select(tadpole, trial, treatment, sline_speed_m_s, PC1, PC2, pond, size, mass, gs, T_initial, camera) %>% ungroup() %>% mutate(treatment = factor(treatment),
                     pond = factor(pond)) %>% 
...
```{r}
mod1 <- lmer(sline_speed_m_s ~ treatment * mass + (1|pond/clutch) + (1|indiv) + (1|camera), data = dat3)
summary(mod1)

# without the camera
modx <- lmer(sline_speed_m_s ~ treatment * mass + (1|pond/clutch) + (1|indiv), data = dat3)
summary(modx)

# exclude mass and camera
modpond <- lmer(sline_speed_m_s ~ treatment + (1|pond/clutch) + (1|indiv), data = dat3)
summary(modpond)

# remove indiv
modpond2 <- lmer(sline_speed_m_s ~ treatment + (1|pond/clutch), data = dat3)
summary(modpond2)
...
mod3 <- lmer(sline_speed_m_s ~ treatment * mass + (1|clutch/indiv), data = dat3) # use this syntax, but remember that clutch already accounts for being nested thin pond. 
...
mod_nostruct <- lme(sline_speed_m_s ~ mass*treatment + gs + T_initial, 
         random = ~ 1 | clutch/indiv, 
         data = dat3_nonas)
summary(mod_nostruct)

mod_varfixed <- lme(sline_speed_m_s ~ mass*treatment + gs + T_initial, 
         random = ~ 1 | clutch/indiv,
         weights = varFixed(~gs),
         data = dat3_nonas)
summary(mod_varfixed)

mod_varident <- lme(sline_speed_m_s ~ mass*treatment + gs + T_initial,
         random = ~ 1 | clutch/indiv,
         weights = varIdent(form = ~ 1 | factor(as.character(gs))),
         data = dat3_nonas)

# mod_varpower <- lme(sline_speed_m_s ~ mass*treatment + gs + T_initial, 
#          random = ~ 1 | clutch/indiv,
#          weights = varPower(form = ~ gs),
#          data = dat3_nonas)
# summary(mod_varpower) # but we shouldn't use this, since gs can take a value of 0 once scaled.

mod_varexp <- lme(sline_speed_m_s ~ mass*treatment + gs + T_initial, 
         random = ~ 1 | clutch/indiv,
         weights = varExp(form = ~ gs),
         data = dat3_nonas)
summary(mod_varexp)

mod_varconstpower <- lme(sline_speed_m_s ~ mass*treatment + gs + T_initial, 
         random = ~ 1 | clutch/indiv,
...
formula(consensus_mod)
wild_mod <- lmer(sline_speed_m_s ~ mass * avg_pond_temp + gs + T_initial + (1 | tadpole), 
...
full <- lmer(sline_speed_m_s ~ PC2*mass*gs*avg_pond_temp + T_initial + (1 | tadpole),
             data = wild_nonas, 
             na.action = "na.fail", 
             REML = F)
...
m1 <- lmer(sline_speed_m_s ~ mass + (1 | tadpole), data = wild_nonas, REML = F, 
           na.action = "na.fail")

m2 <- lmer(sline_speed_m_s ~ mass + avg_pond_temp + (1 | tadpole), data = wild_nonas, REML = F, 
           na.action = "na.fail")

m3 <- lmer(sline_speed_m_s ~ mass + PC2 + (1 | tadpole), data = wild_nonas, REML = F, 
           na.action = "na.fail")

m4 <- lmer(sline_speed_m_s ~ gs + (1 | tadpole), data = wild_nonas, REML = F, 
           na.action = "na.fail")

m5 <- lmer(sline_speed_m_s ~ mass + T_initial + (1 | tadpole), data = wild_nonas, REML = F, 
           na.action = "na.fail")
```
...
reduced_model <- lmer(sline_speed_m_s ~ mass + (1 | tadpole),
                data = wild_nonas,
                na.action = "na.fail",
                REML = F)
