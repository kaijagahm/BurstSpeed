...
checks %>% ggplot(aes(x = stagediff))+
  geom_density(fill = "darkgrey")+ # this is a good sign! We can quantify the error rate later.
...
tads <- dev %>% # create new rows for each tadpole
  ...
  distinct() %>%
  left_join(hatch_tojoin, by = c("pond", "clutch", "treatment")) %>% distinct()
...
with(hatch, table(pond, Coll.Date))
...
```{r}
# Get wild tapole info
wild_info <- read.csv("data/inputs/wild_info.csv", as.is = T)
...
deploy <- deploy %>% 
  mutate(datetime_in = paste(date_in, time_in),
         datetime_out = paste(date_out, time_out)) %>%
...
tempsdown <- temps %>% 
  select(-c(datetime, temp_c, datetime_in, datetime_out)) %>% 
  distinct() %>%
  ungroup() %>%
...
## Visualize
```{r}
tempsdown %>% ggplot(aes(x = date, y = mean_daily_temp_c, col = pond))+
  geom_line(lwd = 1.3)+
...
## Join
```{r}
loggers_tojoin <- tempsdown %>% 
  select(pond, date, mean_daily_temp_c) %>%
  rename(temp_c = mean_daily_temp_c) %>%
  mutate(date = as.POSIXct(date))

incs_long <- incs_long %>%
  mutate(date = as.POSIXct(date))

alltemps <- bind_rows(loggers_tojoin, incs_long)
```

## Visualize
```{r}
loggers_tojoin %>% ggplot(aes(x = date, y = temp_c, group = pond))+
  geom_line()+
