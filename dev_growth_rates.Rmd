...
```{r}
# Embryo collection dates for each pond
collection_dates <- emb %>% 
  select(pond, Coll.Date) %>% 
  distinct() %>%
  rename(emb_collection_date = Coll.Date)
...
# Join embryo collection dates to wild tadpole collection dates
wild_tads <- wild_tads %>%
  left_join(collection_dates, by = "pond") %>%
  mutate(emb_collection_date = dmy(emb_collection_date))

# Get unique pond-interval combos
unique_combos <- wild_tads %>% 
  select(pond, emb_collection_date, coll.date) %>%
  distinct()
nrow(unique_combos)
...
for(i in 1:nrow(unique_combos)){
  tempsvec <- temps %>% 
    filter(pond == unique_combos$pond[i], 
           date >= unique_combos$emb_collection_date[i], 
           date <= unique_combos$coll.date[i]) %>% pull(temp_c)
  avg_temp <- mean(tempsvec, na.rm = T)
  
  unique_combos$avg_pond_temp[i] <- avg_temp
  print(paste("iteration", i, "of", nrow(unique_combos), "complete"))
}

wild_tad_temps <- left_join(wild_tads, unique_combos, by = c("pond", "emb_collection_date", "coll.date"))
wild_tad_temps <- wild_tad_temps %>%
  mutate(tadpole = paste0(tadpole, "_Wild"))

save(wild_tad_temps, file = "data/outputs/wild_tad_temps.Rda")
```

# Boxplots of daily temperatures during the dev. period
```{r}
head(unique_combos)
pond_intervals <- unique_combos %>% 
  group_by(pond) %>% 
  filter(coll.date == max(coll.date)) %>% # get longest interval
  select(-avg_pond_temp)

fortempboxplots <- left_join(temps, pond_intervals, by = "pond") # join to ponds
save(fortempboxplots, file = "data/outputs/fortempboxplots.Rda")

# Crop to the developmental window and get just daily average temperatures
fortempboxplots <- fortempboxplots %>% 
  group_by(pond) %>% 
  filter(date >= emb_collection_date & date <= coll.date) %>%
  select(pond, date, mean_daily_temp_c, emb_collection_date, coll.date) %>%
  distinct()
