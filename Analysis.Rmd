...
Hold all variables constant at their mean values for lab tadpoles except for Gosner stage. Size_at_stage will be held to 0, since those values are residuals.
```{r}
# Load data
load("data/outputs/dat3_nonas.Rda")
load("data/outputs/wild_nonas.Rda")
...
newlab.gs <- dat3_nonas %>% # make new means/medians dataset (lab)
  select(tadpole, clutch, size_at_stage, treatment, pond, T_initial, gs, PC2) %>%
  mutate(T_initial = mean(T_initial),
         size_at_stage = mean(size_at_stage),
         PC2 = mean(PC2))

newwld.gs <- wild_nonas %>%
  select(tadpole, size_at_stage, avg_pond_temp, pond, T_initial, gs, PC2) %>%
  mutate(T_initial = mean(T_initial),
         size_at_stage = mean(size_at_stage),
