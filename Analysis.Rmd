...
Check variance inflation factor for the original models to make sure temp isn't highly multicollinear
```{r}
vif(fullmodel_size)
vif(fullmodel_mass)
vif(fullmodel_gs)
...
```{r}
dat3_nonas %>% ggplot(aes(sample = sline_speed_m_s, col = treatment))+
  stat_qq()

dat3_nonas %>% ggplot(aes(sample = gs, col = treatment))+
  stat_qq()

dat3_nonas %>% ggplot(aes(sample = T_initial, col = treatment))+
  stat_qq()

dat3_nonas %>% ggplot(aes(sample = PC1, col = treatment))+
  stat_qq()

dat3_nonas %>% ggplot(aes(x = PC1, col = treatment, fill = treatment))+
  geom_density(alpha = 0.5)

# Data are pretty much normally distributed, except for the PC1 measurements, which show a significant right skew.
```

Calculate R (ICCC)
```{r}
summary(mod_gs)
indiv_variance <- 1329e-04
clutch_variance <- 3.186e-12
resid_variance <- 1.181e-04
