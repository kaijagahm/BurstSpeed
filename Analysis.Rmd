...
### Visualize
```{r}
indiv_means %>% ggplot(aes(x = indiv_opp_clutch_diff))+
  geom_histogram()+
  theme_minimal()

indiv_means %>% ggplot(aes(x = pond,
                           y = indiv_opp_clutch_diff,
                           col = pond))+
  geom_violin()+
  geom_point()+
  theme_minimal()+
...
morpho_wild_2 <- lmer(sline_speed_m_s ~ size_pc1*tcat + (1|pond))
```
