...
# BO_W_02_Wild is accidentally recorded as BO_W_02_High in allmorpho. Same with PB_W_02_Wild: accidentally recorded as PB_W_02_High.
```
...
```{r}
na_rows <- stats_temp %>% filter(is.na(T_initial))
shouldbena <- expdata %>% filter(is.na(T_initial)) %>% select(tadpole) %>% unique()
problematic_na_rows <- na_rows %>% filter(!(tadpole %in% shouldbena$tadpole)) %>% group_by(fullname) %>% slice(1) %>% select(tadpole, burst_year, burst_month, burst_day, camera, T_initial)


check <- expdata_tojoin %>% filter(tadpole %in% problematic_na_rows$tadpole)
# Hmm, so some of these do have temperatures on the original data sheet. What's the problem?
