...
```{r}
overall <- dat3 %>% select(treatment, pond, clutch, indiv, tadpole, trial, sline_speed_m_s) %>% 
  group_by(treatment) %>%
  mutate(mean = mean(sline_speed_m_s),
                resid = sline_speed_m_s - mean) %>%
  mutate(level = "overall")

indiv <- overall %>% group_by(treatment, indiv) %>% 
  mutate(mean = mean(sline_speed_m_s),
         resid = sline_speed_m_s - mean) %>%
  mutate(level = "indiv")
...
resids <- rbind(overall, indiv, clutch, pond) # put them all together
...
indiv <- overall %>% group_by(treatment, indiv) %>% 
  mutate(mean = mean(sline_speed_m_s),
         resid = sline_speed_m_s - mean) %>% ungroup() %>%
  mutate(level = "indiv")
...
clutch <- indiv %>% 
  select(treatment, pond, clutch, indiv, mean) %>% 
...
resids_cumulative <- bind_rows(overallsmall, indivsmall, clutchsmall, pondsmall)
resids_cumulative$level <- factor(resids_cumulative$level, levels = c("overall", "pond", "clutch", "indiv"))

head(resids_cumulative)
