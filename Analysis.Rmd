...
dat3_nonas <- dat3_nonas %>% mutate_at(.vars = c("PC1", "PC2", "mass", "size", "gs", "size_pc1"), standardize)
```

4. Fit full models with different alternatives for the size/mass variable
```{r}
fullmodel_mass <- lmer(sline_speed_m_s ~ PC1*PC2*mass*treatment + T_initial + (1|clutch/indiv), data = dat3_nonas, na.action = "na.fail") # FAILED TO CONVERGE

fullmodel_size <- lmer(sline_speed_m_s ~ PC1*PC2*size*treatment + T_initial + (1|clutch/indiv), data = dat3_nonas, na.action = "na.fail")

fullmodel_gs <- lmer(sline_speed_m_s ~ PC1*PC2*gs*treatment + T_initial + (1|clutch/indiv), data = dat3_nonas, na.action = "na.fail") # SINGULAR

fullmodel_pc <- lmer(sline_speed_m_s ~ PC1*PC2*size_pc1*treatment + T_initial + (1|clutch/indiv), data = dat3_nonas, na.action = "na.fail") # SINGULAR
```

5. Dredge each of the full models that converged
```{r}
dredge_size_selection <- dredge(global.model = fullmodel_size)
dredge_size_models <- get.models(dredge_size_selection, subset = TRUE)

dredge_gs_selection <- dredge(global.model = fullmodel_gs)
dredge_size_models <- get.models(dredge_gs_selection)

dredge_pc_selection <- dredge(global.model = fullmodel_pc)
dredge_size_models <- get.models(dredge_pc_selection)
```


Could also try binning temperature and putting it in as a random effect.
