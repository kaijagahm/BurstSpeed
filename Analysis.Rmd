...
### Individual means
```{r}
indiv_means <- dat3 %>% group_by(indiv) %>% 
...
# Initial water temp
wild_nonas %>%
  ggplot(aes(x = T_initial, y = sline_speed_m_s))+
  geom_smooth(aes(col = pond), method = "lm", se = F)+
  geom_smooth(method = "lm", se = F, col = "darkgrey", lty = 2)+
  geom_point(aes(col = pond))+
  scale_color_viridis(discrete = TRUE)
...
wild_mod <- lmer(sline_speed_m_s ~ mass + gs + T_initial + (1 | tadpole), data = wild_nonas, REML = F,
                 na.action = "na.fail")
summary(wild_mod)
vif(wild_mod) # all variance inflation factors look okay.
```

### Try other models for wild tadpoles
```{r}
# Remove individual random effect
wm2 <- lm(sline_speed_m_s ~ mass + gs + T_initial, data = wild_nonas,
            na.action = "na.fail")
summary(wm2) # not significant, and not a good fit

# look at pond differences
wild_nonas %>% ggplot(aes(x = pond, y = sline_speed_m_s))+
  geom_boxplot() # suggests more pond differences than in lab populations.

# Based on the above, try a pond random effect with individual
wm3 <- lmer(sline_speed_m_s ~ mass + gs + T_initial + (1 | pond/tadpole),
            data = wild_nonas,
            REML = FALSE,
            na.action = "na.fail")
summary(wm3) # not significant, and pretty small effects.

# Pond random effect without T_initial, and using size instead of mass
wm4 <- lmer(sline_speed_m_s ~ size + gs + (1 | pond/tadpole),
            data = wild_nonas,
            REML = FALSE,
            na.action = "na.fail")
summary(wm4) # not significant

# Same without gosner stage
wm5 <- lmer(sline_speed_m_s ~ size + (1 | pond/tadpole),
            data = wild_nonas,
            REML = FALSE,
            na.action = "na.fail")
summary(wm5) # that's a little better.

# Basic linear model: pond and mass
wm5lm <- lm(sline_speed_m_s ~ mass + pond,
            data = wild_nonas,
            na.action = "na.fail")
summary(wm5lm)

# Fit full model
full <- lmer(sline_speed_m_s ~ PC2*mass*gs + T_initial + (1 | pond/tadpole),
             data = wild_nonas, 
             na.action = "na.fail", 
             REML = F)
...
head(wild_full_dredge)
vif(full) # check variance inflation factors: super high, of course.

# Select reduced model: best AIC
reduced_model <- lmer(sline_speed_m_s ~ mass + (1 | pond/tadpole),
                data = wild_nonas,
                na.action = "na.fail",
                REML = F)
summary(reduced_model)

# This reduced model basically comes down to the same thing as the above linear model with pond and size. Only difference is whether we want to treat pond as a random effect or not. 
summary(reduced_model)
summary(wm5lm)

# Look at a plot of the linear model
sjPlot::plot_model(wm5lm, type = "eff", title = "Predicted speed")
sjPlot::plot_model(reduced_model, type = "eff", title = "Predicted speed") # this one doesn't show the ponds: visual representation of what we already know to be true--we care about the pond-wise differences! 

# We'll go with the linear model on this one then. 
consensus_mod_wild <- wm5lm
plot_model(consensus_mod_wild, type = "std", title = "Coefficients for wild tadpoles: BO as reference")

# Still trying to figure out which plots I want here.
```

### Fit same model to lab tads
```{r}
lab <- lm(sline_speed_m_s ~ size + pond,
            data = dat3_nonas,
            na.action = "na.fail")
summary(lab)
plot_model(lab, type = "eff")
```
...
all <- bind_rows(wild_nonas, dat3_nonas) %>%
  select(-c(predicted_speed, predicted_speed_withresids))

indiv_means <- all %>% 
  group_by(pond, treatment, tadpole) %>%
  summarize(speed = mean(sline_speed_m_s),
            mass = mass[1])
head(indiv_means)
```
