...
wild_nonas %$% table(pond)
dat3_nonas <- dat3_nonas %>%
  mutate(clutch_only = str_extract(clutch, "(?<= )[[:digit:]]{1}"))
dat3_nonas %$% table(pond, clutch_only)

clutch_n <- dat3_nonas %>%
  group_by(clutch) %>%
  summarize(n = n())

clutch_n %>% summarize(mean = mean(n),
                       max = max(n),
                       min = min(n))
...
wild_pond_n %>% summarize(mean = mean(n),
                          max = max(n),
                          min = min(n))
```
...
# Re-order from highest to lowest within each pond, then order from mean highest to mean lowest by pond. Remove the axis ticks. Alternate gray and light backgrounds.  
plasticity %>% filter(!is.na(clutch_only)) %>% ggplot(aes(x = clutch_only, y = D))+
  geom_boxplot()+
  facet_grid(cols = vars(pond))

# Maybe don't even bother with graphing the coefficients. Make a table with the model output. 
# Change the units to cm/sec maybe? Get whole units. 
